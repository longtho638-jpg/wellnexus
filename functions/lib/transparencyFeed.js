...
export const transparencyFeed = functions.onRequest(async (_req, res) => {
    try {
        ...
        const metrics = metSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const partners = partSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const summary = {
            uptime_30d: metrics.reduce((s, m) => s + (m.uptime || 0), 0) / (metrics.length || 1),
            verify_rate_30d: metrics.reduce((s, m) => s + (m.verifyRate || 0), 0) / (metrics.length || 1),
