...
export class SeedMetricsAdapter {
   ...
   * @param {import('firebase/app').FirebaseApp=} options.app - optional
   * @param {import('firebase/firestore').Firestore=} options.db - optional
   * @param {import('firebase/functions').Functions=} options.functions - optional
   * @param {string=} options.callableName - default: 'getMetricsSummary'
   * @param {string=} options.collectionPath - default: 'metrics'
   * @param {string=} options.docId - default: 'global'
   * @param {number=} options.cacheTtlMs - default: 30_000 ms
   * @param {(msg: string, extra?: any)=>void=} options.logger - optional logger
   */
  constructor(options = {}) {
    const {
      app,
  ...
  /**
   * Lấy metrics 1 lần, từ nguồn chỉ định: 'functions' | 'firestore'.
   * @param {'functions'|'firestore'} source
   * @param {Record<string, string | [string, Function] | Function>=} mapping
   * @param {Object=} opts
   * @param {boolean=} opts.force - bỏ qua cache
  ...
  /**
   * Đăng ký realtime từ Firestore (onSnapshot). Trả về hàm unsubscribe.
   * @param {Object} params
   * @param {Record<string, string | [string, Function] | Function>=} params.mapping
   * @param {(mapped: Record<string, any>, raw: any)=>void} params.onData
  ...
  subscribe({ mapping = defaultMapping, onData, onError } = {}) {
    ...
    this._ensureFirestoreReady(true)
      .then(({ onSnapshot, doc, docRef }) => {
        if (this._unsub) this._unsub();
        this._unsub = onSnapshot(docRef(this._db, this._collectionPath, this._docId), (snapshot) => {
  ...
  async _fetchFromFunctions() {
    ...
    try {
      ...
      // Fallback sang Firestore nếu functions lỗi (ví dụ CORS hoặc chưa deploy)
  ...
  async _fetchFromFirestore() {
    ...
    try {
      ...
      // Nếu không có updatedAt, gán tạm thời để UI có timestamp tham chiếu (không ghi DB)
  ...
  async _ensureFunctionsReady() {
    ...
    // Bạn có thể gán region theo hạ tầng
    this._functions = getFunctions(this._app, undefined); // region mặc định của project
  ...
  async _ensureFirestoreReady(returnHelpersOnly = false) {
    ...
    const helpers = await this._importFirestoreHelpers();
    return returnHelpersOnly ? helpers : helpers;
  }

  async _ensureAppReady() {
    if (this._app) return;
    const { getApps, getApp, initializeApp } = await this._dynamicImport('firebase/app');
    const apps = getApps();
    if (apps.length) {
      this._app = getApp();
    } else {
      // Không khởi tạo app nếu không có config; ném lỗi để caller tự xử lý.
  ...
  async _importFunctionsHelpers() {
    const mod = await this._dynamicImport('firebase/functions');
    return { httpsCallable: mod.httpsCallable };
  }

  async _importFirestoreHelpers() {
    const mod = await this._dynamicImport('firebase/firestore');
    return {
      getDoc: mod.getDoc,
      doc: mod.doc,
      onSnapshot: mod.onSnapshot,
      serverTimestamp: mod.serverTimestamp,
      docRef: mod.doc, // alias cho rõ nghĩa khi subscribe
    };
  }

  async _dynamicImport(spec) {
    // import() dynamic để làm việc trong cả bundler & runtime ESM.
    try {
...
// ------------------------------------------------------------
// Small helper: build mapping nhanh từ danh sách cặp [destKey, path]
// ------------------------------------------------------------
export function buildSimpleMapping(pairs) {
  return Object.fromEntries(pairs.map(([dest, path]) => [dest, path]));
}
