...
const autoRefreshFilter = document.getElementById("auto-refresh");
let isLoading = false;
...
const normalizeAutoRefresh = value => {
  if (typeof value !== "string") {
    return "";
  }
  return Object.prototype.hasOwnProperty.call(AUTO_REFRESH_PRESETS, value) ? value : "";
};
...
const updateQueryString = () => {
  ...
  if (state.autoRefresh) {
    url.searchParams.set("auto_refresh", state.autoRefresh);
  } else {
...
const scheduleAutoRefresh = () => {
  stopAutoRefresh();
  const intervalKey = state.autoRefresh;
  const duration = AUTO_REFRESH_PRESETS[intervalKey];
  if (!duration) {
    return;
  }
...
const loadMetrics = async () => {
  ...
  try {
    ...
    const autoRefreshSummary = describeAutoRefresh(state.autoRefresh);
    const autoRefreshNote = autoRefreshSummary ? ` Auto refresh ${autoRefreshSummary}.` : "";
    const successMessage = metrics.length
      ? `Showing ${metrics.length} partner${metrics.length > 1 ? "s" : ""}${
          timeframeSummary ? ` ${timeframeSummary}` : ""
        }${limitSummary}. Last refreshed at ${refreshedAt}.${autoRefreshNote}`
      : state.updatedWithin
      ? `No partner metrics matched the selected timeframe.${autoRefreshNote}`
      : `No partner metrics returned for this filter.${autoRefreshNote}`;
    ...
    if (consecutiveFailures >= MAX_AUTO_REFRESH_FAILURES && state.autoRefresh) {
      stopAutoRefresh();
      state.autoRefresh = "";
      updateQueryString();
      if (autoRefreshFilter) {
        autoRefreshFilter.value = "";
      }
      setStatus(
        `${baseMessage} Auto refresh has been paused after multiple failures. Please investigate and re-enable when ready.`,
        "error"
      );
      return;
    }

    setStatus(baseMessage, "error");
  } finally {
    if (requestToken === activeRequestToken) {
...
state.autoRefresh = normalizeAutoRefresh(params.get("auto_refresh") || "");
...
if (autoRefreshFilter) {
  autoRefreshFilter.value = state.autoRefresh;
  autoRefreshFilter.addEventListener("change", event => {
    const select = event.target;
    if (!(select instanceof HTMLSelectElement)) {
      return;
    }
    state.autoRefresh = normalizeAutoRefresh(select.value);
    updateQueryString();
    scheduleAutoRefresh();
    if (!isLoading) {
      loadMetrics();
